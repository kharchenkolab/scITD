...
#' @param counts matrix Gene by cell matrix of counts. Use this if want to add batch at
#' count level. (default=NULL)
#' @param nbatches numeric The number of batches to create. Donors are randomly
#' assigned to batches. (default=3)
#' @param batch_override list A list of vectors soecifying donors in each batch
#' (default=NULL)
#'
#' @return either project container with batch added to normalized data or the counts
#' matrix with batch effect added to it
#' @export
get_hybrid_sim <- function(container,counts=NULL,nbatches=3,batch_override=NULL) {
  if (!is.null(counts)) {
    # first normalize by lib size
    lib_sizes <- Matrix::colSums(counts)
    counts <- sweep(counts,MARGIN=2,lib_sizes,FUN='/') * 10000
    container$scMinimal_full$data_sparse <- counts
  }
  
  ## can also later try only making some genes affected by batch
  nGenes <- nrow(container$scMinimal_full$data_sparse)
  batch_factors <- c()
  for (i in 1:nbatches) {
    # batch.facs <- splatter:::getLNormFactors(nGenes, 1, 0.5, .1, .1)
    # batch.facs <- splatter:::getLNormFactors(nGenes, 1, 0.5, .25, .1)
    batch.facs <- splatter:::getLNormFactors(nGenes, 1, 0.5, .5, .1)
  ...
  rownames(batch_factors) <- rownames(container$scMinimal_full$data_sparse)
  ...
  for (i in 1:nbatches) {
    ...
    data_alter <- container$scMinimal_full$data_sparse[,cells_alter]
    data_alter <- data_alter * batch_factors[,batch_name]
    new_dat <- cbind(new_dat,data_alter)
  }
  # reorder columns to match original dataframe
  new_dat <- new_dat[,colnames(container$scMinimal_full$data_sparse)]
  container$scMinimal_full$data_sparse <- new_dat
  
  if (!is.null(counts)) {
    # need to log-transform normlized counts
    container$scMinimal_full$data_sparse <- log1p(new_dat)
  }
